...
export default function SliderSection({ productArr, settings }) {
    return (
        <div className="relative mt-9 h-auto lg:px-[140px] md:px-[10px] mx-auto">
            <div className="header-section text-black flex justify-between h-20 py-5 px-10 mb-2">
                ...
                <button className="text-white md:text-sm sm:text-xs outline-none border-none bg-[#f68122] lg:hover:scale-95 md:hover:scale-100">See all</button>
            </div>
            {
                            ...
                            productArr.map((item, index) => {
                                return (
                                    <div key={index} className="h-[325px] flex flex-wrap md:px-[30px] sm:px-[10px] py-6">
                                        <div className="rounded-2xl overflow-hidden cursor-pointer" style={{ boxShadow: '1px 1px 13px 0px #00000040' }}>
                                            <img src={(item.image)} className="h-auto max-w-full" />
                                        </div>
                                        <div className="h-[60px] text-black mt-2">
                                            <label className="font-medium text-base cursor-pointer hover:text-[#f68122]">{item.name}</label><br></br>
